from src.utils.calculation import (
    calculate_project_payments,
    calculation_workers_operations,
    calculation_workers_salaries,
)
...
def build_workers_dict(row):
    ...
    (
        data["total_number_of_working_hours"],
        data["total_payment_amount"],
    ) = calculation_workers_operations(row[0])
    data["rest_amount"] = data["total_payment_amount"] - data["total_payed_amount"]
    return data
